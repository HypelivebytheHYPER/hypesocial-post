...
export async function POST(request: NextRequest) {
  // Support both direct Post For Me calls (POST_FOR_ME_WEBHOOK_SECRET)
  // and Cloudflare Worker forwarded calls (NEXTJS_WEBHOOK_SECRET)
  const expectedSecrets = [
    process.env.POST_FOR_ME_WEBHOOK_SECRET,
    process.env.NEXTJS_WEBHOOK_SECRET,
  ...
  const isValid = webhookSecret && expectedSecrets.some((secret) =>
    secureCompare(webhookSecret, secret)
  );
